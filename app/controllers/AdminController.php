...
class AdminController {
    ...
    public function add_therapist() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($passwordErrors)) {
                $msg = '<div style="color:red;margin-bottom:1rem;">Password must contain: ' . implode(', ', $passwordErrors) . '.</div>';
            } else {
                ...
                $result = Therapist::register($name, $email, $phone, $password, true); // autoApprove = true for admin
                if ($result === true) {
    ...
    public function add_appointment() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($appointment_id) {
                header('Location: /public/index.php?page=admin_add_appointment&success=1&customer_name=' . urlencode($name));
                exit;
            } else {
