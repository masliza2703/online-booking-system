...
class Therapist {
    public static function register($name, $email, $phone, $password, $autoApprove = false) {
        global $pdo;
        ...
        $stmt = $pdo->prepare('INSERT INTO therapists (therapist_name, therapist_email, therapist_phone_number, therapist_password, is_approved, status) VALUES (?, ?, ?, ?, ?, ?)');
        if ($stmt->execute([$name, $email, $phone, $hash, $is_approved, $status])) {
            return true;
        }
