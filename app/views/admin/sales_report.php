...
// Handle "none" option for therapist and service
if ($selected_therapist === 'none') {
  ...
  <div style="
    ...
    <main style="flex:1;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
        ...
        <div style="max-width:900px;margin:2.5rem auto;padding:2.5rem 2rem;background:#fff;border-radius:1.5rem;box-shadow:0 2px 12px rgba(0,0,0,0.07);">
          ...
          <div style="background:#f8f9fa;border-left:4px solid #3b5998;padding:1rem;margin-bottom:1.5rem;border-radius:0 8px 8px 0;">
            <div style="font-weight:500;color:#232a32;margin-bottom:0.5rem;">📊 Report Options:</div>
            <div style="font-size:0.9rem;color:#666;line-height:1.4;">
              • <strong>Filter dropdowns:</strong> Select specific therapist/service to filter the data<br>
              • <strong>Hide checkboxes:</strong> Check to hide service/therapist columns from the report<br>
              • <strong>Date range:</strong> Required to generate the report<br>
              • <strong>Default:</strong> Report shows all columns unless hidden
            </div>
          </div>
          <?php if ($selected_therapist && $show_report): ?>
            <?php 
            $selected_therapist_name = '';
            if ($selected_therapist === 'none') {
                $selected_therapist_name = 'No Therapist Assigned';
            } else {
                foreach ($therapists as $therapist) {
                    if ($therapist['therapist_id'] == $selected_therapist) {
                        $selected_therapist_name = $therapist['therapist_name'];
                        break;
                    }
                }
            }
            ?>
            <div style="background:#e3f2fd;color:#1565c0;padding:1rem 1.5rem;border-radius:10px;margin-bottom:1.5rem;text-align:center;font-weight:500;">
              Showing sales for therapist: <strong><?= htmlspecialchars($selected_therapist_name) ?></strong>
            </div>
          <?php endif; ?>
          <?php if ($selected_service && $show_report): ?>
            <?php 
            $selected_service_name = '';
            if ($selected_service === 'none') {
                $selected_service_name = 'No Service Assigned';
            } else {
            ...
            ?>
            <div style="background:#e8f5e8;color:#2e7d32;padding:1rem 1.5rem;border-radius:10px;margin-bottom:1.5rem;text-align:center;font-weight:500;">
              Showing sales for service: <strong><?= htmlspecialchars($selected_service_name) ?></strong>
            </div>
          <?php endif; ?>
          <form method="get" style="display:flex;gap:1.2rem;align-items:end;margin-bottom:2rem;flex-wrap:wrap;">
            ...
            <div>
              <label for="therapist_id" style="font-weight:500;">Therapist:</label>
              <select id="therapist_id" name="therapist_id" style="padding:0.45rem 0.8rem;border:1px solid #bfc9d9;border-radius:6px;font-size:1rem;min-width:150px;">
                <option value="">All Therapists</option>
                <option value="none" <?= ($selected_therapist == 'none') ? 'selected' : '' ?>>None</option>
                <?php foreach ($therapists as $therapist): ?>
                  <option value="<?= $therapist['therapist_id'] ?>" <?= ($selected_therapist == $therapist['therapist_id']) ? 'selected' : '' ?>>
            ...
            <div>
              <label for="service_id" style="font-weight:500;">Service:</label>
              <select id="service_id" name="service_id" style="padding:0.45rem 0.8rem;border:1px solid #bfc9d9;border-radius:6px;font-size:1rem;min-width:150px;">
                <option value="">All Services</option>
                <option value="none" <?= ($selected_service == 'none') ? 'selected' : '' ?>>None</option>
                <?php foreach ($services as $service): ?>
                  <option value="<?= $service['service_id'] ?>" <?= ($selected_service == $service['service_id']) ? 'selected' : '' ?>>
            ...
            <div style="display:flex;align-items:center;gap:0.7rem;">
              <label><input type="checkbox" name="hide_service" value="1" <?= isset($_GET['hide_service']) ? 'checked' : '' ?>> Hide Service Column</label>
              <label><input type="checkbox" name="hide_therapist" value="1" <?= isset($_GET['hide_therapist']) ? 'checked' : '' ?>> Hide Therapist Column</label>
            </div>
          ...
          <form method="post" style="display:flex;gap:1rem;margin-bottom:1.5rem;">
            ...
            <input type="hidden" name="therapist_id" value="<?= htmlspecialchars($selected_therapist) ?>">
            <input type="hidden" name="service_id" value="<?= htmlspecialchars($selected_service) ?>">
            <input type="hidden" name="hide_therapist" value="<?= isset($_GET['hide_therapist']) ? '1' : '' ?>">
            <input type="hidden" name="hide_service" value="<?= isset($_GET['hide_service']) ? '1' : '' ?>">
            ...
            <table style="width:100%;border-collapse:collapse;font-size:1.08rem;">
              <thead>
                <tr style="background:#e6eaf3;color:#232a32;">
                  ...
                  <?php if (!isset($_GET['hide_service'])): ?><th style="padding:1rem 0.5rem;text-align:center;">Service</th><?php endif; ?>
                  <?php if (!isset($_GET['hide_therapist'])): ?><th style="padding:1rem 0.5rem;text-align:center;">Therapist</th><?php endif; ?>
              ...
              <tbody>
                ...
                <tr style="background:<?= $i%2==0 ? '#f8fafd' : '#fff' ?>;">
                  ...
                  <?php if (!isset($_GET['hide_service'])): ?><td style="padding:0.9rem 0.5rem;text-align:center;"> <?= htmlspecialchars($s['service_name']) ?> </td><?php endif; ?>
                  <?php if (!isset($_GET['hide_therapist'])): ?><td style="padding:0.9rem 0.5rem;text-align:center;"> <?= htmlspecialchars($s['therapist_name']) ?> </td><?php endif; ?>
                ...
                <?php endforeach; else: ?>
                <tr><td colspan="<?= 4 + (!isset($_GET['hide_service'])?1:0) + (!isset($_GET['hide_therapist'])?1:0) ?>" style="text-align:center;padding:1.5rem;">No sales found.</td></tr>
                <?php endif; ?>
              </tbody>
              <tfoot>
                <tr style="background:#e6eaf3;font-weight:bold;">
                  <td colspan="<?= 3 + (!isset($_GET['hide_service'])?1:0) + (!isset($_GET['hide_therapist'])?1:0) ?>" style="text-align:right;padding:1rem 0.5rem;">Total</td>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Optional: Add some helpful behavior for the hide checkboxes
  var hideTherapistCheckbox = document.querySelector('input[name="hide_therapist"]');
  var hideServiceCheckbox = document.querySelector('input[name="hide_service"]');
  
  // Show tooltip or hint when hovering over hide checkboxes
  if (hideTherapistCheckbox) {
    hideTherapistCheckbox.title = "Check this to hide the Therapist column from the report";
  }
  if (hideServiceCheckbox) {
    hideServiceCheckbox.title = "Check this to hide the Service column from the report";
  }
});
</script> 