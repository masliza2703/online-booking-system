...
window.addEventListener('DOMContentLoaded', function() {
  Swal.fire({
    ...
    html: `
      ...
      <div style="margin-top:1.5rem;padding:1rem;background:#fff3cd;border:2px solid #ffc107;border-radius:10px;font-weight:bold;color:#856404;">
        <div style="margin-bottom:0.5rem;">⚠️ <strong>IMPORTANT:</strong></div>
        Please say your name <strong style="color:#1976d2;font-size:1.1em;">"<?= htmlspecialchars($appointmentDetails['name']) ?>"</strong> during appointment's day.
      </div>
    `,
    ...
    bookingForm.addEventListener('submit', function(e) {
      ...
      if (phoneInput && !/^\d+$/.test(phoneInput.value)) {
        Swal.fire({
          icon: 'error',
          title: 'Ralat',
          text: 'Number phone must contain only numbers.'
        });
        phoneInput.focus();
        e.preventDefault();
      }
