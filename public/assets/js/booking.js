...
window.addEventListener('DOMContentLoaded', function() {
    if (window.flatpickr) {
        ...
        if (dateStr === new Date().toISOString().split('T')[0]) {
            ...
            const bufferHour = currentHour + 1; // This will be handled in renderTimeSlotsForDay function
        }
            }
        });
    }
    ...
    function renderTimeSlotsForDay(day) {
        ...
        // Round start time to nearest 15-minute slot
        let roundedStart = Math.ceil(adjustedStart / 15) * 15;
        if (roundedStart < adjustedStart) {
            roundedStart += 15;
        }

        // Calculate latest possible start time (end time - 15 min buffer)
        const latestStart = end - 15;
        const times = [];
        
        for (let mins = roundedStart; mins <= latestStart; mins += 15) {
...
Date: ${date}<br>
Time: ${time}<br>
${addonTherapist.length ? `Add-on Therapist: ${addonTherapist.join(', ')}<br>` : ''}
${addonService.length ? `Add-on Service: ${addonService.join(', ')}<br>` : ''}
<br>
Please check the details before confirming the appointment.
                `,
...
Time: ${time}<br>
        ...
        btnConfirm.addEventListener('click', function() {
            ...
            .catch(() => {
                adminForm.parentNode.innerHTML = '<p>Sorry, something went wrong. Please try again.</p>';
            });
        });
    }
})();
...
if (!document.getElementById('admin-time-slot-style')) {
    ...
    style.innerHTML = `
        .admin-time-slot-btn.selected {
            background: #9c6b53 !important;
            color: #fff !important;
            border: 2px solid #7a4c2e !important;
        }
        .admin-time-slot-btn:disabled {
