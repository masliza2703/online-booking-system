...
if (isset($_POST['download']) && isset($_GET['page']) && $_GET['page'] === 'admin_sales_report') {
    ...
    if ($_POST['download'] === 'excel') {
        ...
        $total = 0;
        echo '<html><head><title>Sales Report</title><link rel="icon" type="image/x-icon" href="/public/assets/images/logo.jpg"><link rel="icon" type="image/png" sizes="32x32" href="/public/assets/images/logo.jpg"><link rel="icon" type="image/png" sizes="16x16" href="/public/assets/images/logo.jpg"><link rel="apple-touch-icon" href="/public/assets/images/logo.jpg"><meta name="msapplication-TileImage" content="/public/assets/images/logo.jpg"><style>body{font-family:Arial,sans-serif;}table{border-collapse:collapse;width:100%;margin-top:16px;}th,td{border:1px solid #ccc;padding:8px;text-align:center;}th{background:#eee;} .info-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;} .info-block{font-size:1rem;text-align:left;} .total-block{font-size:1.1rem;text-align:right;font-weight:bold;}</style></head><body>';
        ...
        $redirect_url = '/public/index.php?page=admin_sales_report&generate=1&start_date=' . htmlspecialchars($start_date) . '&end_date=' . htmlspecialchars($end_date);
        if (!empty($therapist_id) || $therapist_id === 'none') {
            $redirect_url .= '&therapist_id=' . htmlspecialchars($therapist_id);
        }
        if (!empty($service_id) || $service_id === 'none') {
            $redirect_url .= '&service_id=' . htmlspecialchars($service_id);
        }
        echo '<script>window.onload=function(){window.print();setTimeout(function(){window.location.href=\'' . $redirect_url . '\';},1000);}</script>';
        echo '</body></html>';
        exit;
    }
}
